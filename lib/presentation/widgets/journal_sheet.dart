...
import 'puff_ui.dart'; // PrimaryCta и т.д.
...
class JournalSheet {
  static Future<bool> show(BuildContext context) async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (_) {
        // Важно: передаём существующий BLoC в sheet
        return BlocProvider.value(
          value: context.read<JournalBloc>(),
          child: const _JournalSheetBody(),
        );
      },
    );
...
class _JournalSheetBodyState extends State<_JournalSheetBody> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // Даём BLoC чуть времени обработать (или можно слушать state)
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 8,
                  children: [
                    _tag(ru ? 'стресс' : 'stress', 'stress', isDark),
                    _tag(ru ? 'еда' : 'food', 'food', isDark),
                    _tag(ru ? 'люди' : 'people', 'people', isDark),
                    _tag(ru ? 'работа' : 'work', 'work', isDark),
                    _tag(ru ? 'тяга' : 'craving', 'craving', isDark),
                    _tag(ru ? 'победа' : 'win', 'win', isDark),
                  ],
                ),
  ...
  Widget _tag(String label, String id, bool isDark) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected
              ? AppColors.accent.withValues(alpha: isDark ? 0.15 : 0.11)
              : isDark
              ? Colors.white.withValues(alpha: 0.035)
              : Colors.black.withValues(alpha: 0.025),
